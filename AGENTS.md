...
Use Vitest with Testing Library and jsdom. Place `*.test.tsx` component tests under `tests/`, mirroring source organization when practical. Run `npm test` for a single pass or `npm run test:watch` during development. Test user-visible behavior and bug fixes, and keep tests independent of private credentials. No coverage threshold is configured.
